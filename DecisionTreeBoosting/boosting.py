...
def gradient_logistic(train_target, train_predict):
    ...
    """
    denominator = 1 + np.exp(train_target * train_predict)
    grad = -train_target / denominator
    return grad
...
class GradientBoosting:
    ...
    def fit(self, train_data, train_target):
        ...
        for t in range(self.T):
            rgs = DecisionTreeRegressor(
                min_samples_split=self.min_sample, max_depth=self.max_depth
            )
            gt = self.gradient_func(train_target, ft)  # g_t
            rgs.fit(train_data, -gt)
            self.ht.append(rgs)
    ...
    def predict(self, test_data):
        ...
        for h in self.ht:
            pred += self.learning_rate * h.predict(test_data)
        return pred
...
def training_boosting_classifier():
    ...
    for idx, i, tt in zip(
        ...
        gbt = GradientBoosting(
            T=i, gradient_func=gradient_l2, max_depth=2, learning_rate=0.1
        )
        ...
        axarr[idx[0], idx[1]].scatter(
            x_train[:, 0], x_train[:, 1], c=y_train_label, alpha=0.8
        )
        axarr[idx[0], idx[1]].set_title(tt)
    plt.savefig("output/GBM_l2.png", dpi=300)
    ...
    for idx, i, tt in zip(
        ...
        gbt = GradientBoosting(
            T=i, gradient_func=gradient_logistic, max_depth=3, learning_rate=0.1
        )
        gbt.fit(x_train, y_train)
        ...
        axarr[idx[0], idx[1]].set_title(tt)
    plt.savefig("output/GBM_logistic.png", dpi=300)
...
def training_boosting_regression():
    ...
    x_krr_test, y_krr_test = data_krr_test[:, 0].reshape(-1, 1), data_krr_test[
        :, 1
    ].reshape(-1, 1)

    plot_size = 0.001
    x_range = np.arange(0.0, 1.0, plot_size).reshape(-1, 1)
    ...
    plt.savefig("output/GBM_regression.png", dpi=300)
