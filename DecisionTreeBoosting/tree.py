...
def training_classification_tree():
    ...
    for idx, clf, tt in zip(
        product([0, 1], [0, 1, 2]), clfs, ["Depth = {}".format(n) for n in range(1, 7)]
    ):
    ...
    plt.savefig("output/DT_entropy.png", dpi=300)
...
def training_regression_tree():
    ...
    for idx, reg, tt in zip(
        product([0, 1], [0, 1, 2]), regs, ["Depth = {}".format(n) for n in range(1, 7)]
    ):
        y_range_predict = np.array([reg.predict_instance(x) for x in x_range]).reshape(
            -1, 1
        )
    ...
    plt.savefig("output/DT_regression.png", dpi=300)
